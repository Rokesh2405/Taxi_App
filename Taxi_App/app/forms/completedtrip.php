...
$datalist=array();
$checkvaliduser = $db->prepare("SELECT * FROM `booking` WHERE `register_id`='".$data->register_id."' AND `completed_status`='1' AND `total_amount_to_pay`!='' ORDER BY `trip_date` ASC, `trip_time` ");
...
$daddress=$row['drop_address'];
	...
	$invoice="https://droptaximadurai.in/appadmin/MPDF/trip_invoice.php?id=".$row['id'];
 //if($bbchecknum>0) {
