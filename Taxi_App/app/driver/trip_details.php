...
if(
 ...
 $rowss11 = $checkvaliduser->fetch(PDO::FETCH_ASSOC);
 $booking_id=$data->notification_id;
 
if($data->type=='start') { 
    
        //update read status to driver
$readquery = "UPDATE `notification` SET `read_status`='1' WHERE `to`='".$rowss11['id']."' AND `booking_id`='".$data->notification_id."'  ";
$readstmt = $db->prepare($readquery);
$readstmt->execute();
//update read status

    //update read status to customer
$readquery = "UPDATE `notification` SET `read_status`='1' WHERE `booking_id`='".$data->notification_id."' AND `from`='admin' ";
$readstmt = $db->prepare($readquery);
$readstmt->execute();
//update read status
...
$query = "UPDATE `booking` SET
                    driver_id='".$rowss11['id']."',start_km='".$data->km."',start_image='".$profimg."',pickup_time='".$data->triptime."' WHERE id='".$data->notification_id."'";
...
$message="Hello, I have reached the pickup location and the trip is started.";

$messagenoti="Hello, I have reached the pickup location and the trip is started.";

$query = "INSERT INTO `notification` SET
`booking_id`='".$data->notification_id."',`from`='".$rowss11['id']."',`to`='admin',`title`='".$title1."',`message`='".$message."',`type`='Driver-Admin' ";
$stmt = $db->prepare($query);
$stmt->execute();
$lasid=$db->lastInsertId();
 ...
 $getdriver = $db->prepare("SELECT * FROM `notification` WHERE `booking_id`='".$data->notification_id."' AND `driver_charge`!='' AND `confirm_status`='1' ORDER BY `id` ASC");
$getdriver->execute();
...
$messagenoti="Hello, I have reached the pickup location and the trip is started.";
$query = "INSERT INTO `notification` SET
`booking_id`='".$data->notification_id."',`from`='admin',`to`='".$getdriverdetails['from']."',`title`='".$title1."',`message`='".$message."',`type`='Admin-Driver' ";
$stmt = $db->prepare($query); 
$stmt->execute();
 $title="DROPTAXI - Booking No :".$data->notification_id;
...
$query = "UPDATE `booking` SET
                    end_km='".$data->km."',drop_time='".$data->triptime."',waiting_hours='".$data->waiting_hours."' WHERE id='".$data->notification_id."'";
$stmt = $db->prepare($query);
$stmt->execute();
//$paidamount=getbookingdetails('customer_booking_amount',$data->notification_id);
$paidamount=getbookingdetails('customer_paid_booking_amount',$data->notification_id);
$startkm=getbookingdetails('start_km',$data->notification_id);
$triptpe=getbookingdetails('triptype',$data->notification_id);
$carid=getbookingdetails('car_id',$data->notification_id);
$endkm=$data->km;
$distance=$endkm-$startkm;
$bookid=$data->notification_id;
	$toll_extra_fess=$data->toll_extra_fess;
$kmresponse=balancetripamount($triptpe,$carid,$distance,$paidamount,$waiting_hours,$bookid,$toll_extra_fess);
