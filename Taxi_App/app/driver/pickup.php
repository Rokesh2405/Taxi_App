...
if($checknum>0) {
if(
    !empty($data->driver_id) &&  !empty($data->booking_id) &&  !empty($data->triptime) &&  !empty($data->type)
){
    if($data->type=='pickup') { 
  $uquery = "UPDATE `booking` SET
                    pickup_time='".$data->triptime."' WHERE `id`='".$data->booking_id."' ";
    }
    else
    {
        $uquery = "UPDATE `booking` SET
                    drop_time='".$data->triptime."' WHERE `id`='".$data->booking_id."' ";
    }
