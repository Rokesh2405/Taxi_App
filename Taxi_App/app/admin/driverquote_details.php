...
http_response_code(200);
    ...
    echo json_encode(
        array("success" => "true","error" => "false","notification_id"=>$id,"booking_id"=>$booking_id,"car_image"=>$img,"driver_id"=>$from,"drivername"=>getdriver('driver_name',$from),"driver_mobileno" => getdriver('driver_mobileno',$from),"licenceno" => getdriver('licence_no',$from),"quoate_amount" =>$driver_charge,"pickup_address" => getbookingdetails('pickup_address',$booking_id),"drop_address" => getbookingdetails('drop_address',$booking_id))
    ); 
}
else
{
