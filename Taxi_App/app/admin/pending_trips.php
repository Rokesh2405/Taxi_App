...
$stmt11 = $db->prepare("SELECT * FROM `booking` WHERE `total_amount_to_pay` IS NULL ORDER BY `id` DESC");	
$stmt11->execute();
$checknum11 = $stmt11->rowCount();
if($checknum11>0) {
while ($row11 = $stmt11->fetch(PDO::FETCH_ASSOC)){

$stmt12 = $db->prepare("SELECT * FROM `notification` WHERE `booking_id`='".$row11['id']."' AND `confirm_status`='1' ORDER BY `id` DESC");	
$stmt12->execute();
