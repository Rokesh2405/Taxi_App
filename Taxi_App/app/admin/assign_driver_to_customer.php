...
// $stmt12 = $db->prepare("SELECT * FROM `notification` WHERE `booking_id`='".$data->booking_id."' AND `driver_name` IS NULL AND `from`='".$data->driver_id."' ORDER BY `id` DESC");	
// $stmt12->execute();
// $row113 = $stmt12->fetch(PDO::FETCH_ASSOC);
   // Notification 
   
   //update read status
$readquery = "UPDATE `notification` SET `read_status`='1' WHERE `booking_id`='".$data->booking_id."' AND `from`='".$data->driver_id."'";
...
$notification = new Notification();
                    ...
                    $message="Hi ".getdriver('name',$data->driver_id).", Thank you accepting booking. Please send us your car type, car number, and your mobile number.";
                    $messagenoti="Hi ".getdriver('name',$data->driver_id).", Thank you accepting booking. Please send us your car type, car number, and your mobile number.";
                     
					$query = "INSERT INTO `notification` SET
                    `booking_id`='".$data->booking_id."',`from`='admin',`to`='".$data->driver_id."',`title`='".$title1."',`message`='".$message."',`type`='Admin-Driver' ";
$stmt = $db->prepare($query);
$stmt->execute();

$lastid=$db->lastInsertId();

$title="DROPTAXI - Bookingno :".$data->booking_id;
//	$title="DROPTAXI - Bookingno :".$row113['id'];
