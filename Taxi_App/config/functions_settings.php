...
function addusers($kit,$orderlist,$image,$hostid,$tamil_state,$english_state,$appurl,$userid, $username, $password,$status,$users,$getid) {
 
    global $db;
    if ($getid == '') {
        $link22 = FETCH_all("SELECT `id` FROM `users` WHERE `val1`=?", $username);
        if ($link22['id'] == '') {
           // $ress="INSERT INTO `users` (`name`, `val1`,`val2`,`val3`) VALUES('".$name."','".$username."','".$password."','".$status."' )";
            $resa = $db->prepare("INSERT INTO `users` (`image`,`hostid`,`name`, `tamil_state`,`english_state`,`appurl`,`val1`,`val2`,`val3`,`orgpassword`) VALUES(?,?,?,?,?,?,?,?,?,?)");
            $resa->execute(array($image,$hostid,$userid,$tamil_state,$english_state,$appurl,$username, md5($password), $status,$password));
            
            $id=$db->lastInsertId();
            $cinid='';
            $tinid='';
           
            $resa1 = $db->prepare("INSERT INTO `permission` (`kit`,`orderlist`,`users`,`userid`) VALUES(?,?,?,?)");
            $resa1->execute(array($kit,$orderlist,$users,$id));
        ...
        if ($link22['id'] == '') {
            ...
            $resa = $db->prepare("UPDATE `users` SET `image`=?,`hostid`=?,`tamil_state`=?,`english_state`=?,`appurl`=?,`name`=?,`val1`=?,`val3`=? WHERE `id`=?");
            $resa->execute(array($image,$hostid,trim($tamil_state),trim($english_state),trim($appurl),trim($userid),trim($username),trim($status), $getid));
                     
           
            
            $resa = $db->prepare("UPDATE `permission` SET `kit`=?,`orderlist`=?,`users`=? WHERE `userid`=?");
            $resa->execute(array($kit,$orderlist,$users, $getid));
...
function getpermission($a, $b) {
    global $db;
    $get1 = $db->prepare("SELECT * FROM `permission` WHERE `userid`=?");
    $get1->execute(array($b));
    $get = $get1->fetch(PDO::FETCH_ASSOC);
    $res = $get[$a];
    return $res;
...
function addsendgrid($a, $b, $c,$f,$g, $d, $e) {
    ...
    if ($e == '') {
        ...
        $htry = $db->prepare("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES (?,?,?,?,?,?)");
        $htry->execute(array('sendgrid', 41, 'Insert', $_SESSION['ALOID'], $d, $insert_id));
        ...
        $htry = $db->prepare("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES (?,?,?,?,?,?)");
        $htry->execute(array('sendgrid', 41, 'Update', $_SESSION['ALOID'], $d, $e));
        $res = '<div class="alert alert-success alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button><h4><i class="icon fa fa-check"></i> Successfully Updated!</h4></div>';
    }
    return $res;
}
...
function addemailtemplate($subject, $description, $order, $status, $ip, $getid) {
    ...
    if ($getid == '') {
        ...
        $htry = $db->prepare("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES (?,?,?,?,?,?)");
        $htry->execute(array('Email Template', 41, 'Insert', $_SESSION['ALOID'], $d, $insert_id));
        ...
        $htry = $db->prepare("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES (?,?,?,?,?,?)");
        $htry->execute(array('Email Template', 41, 'Update', $_SESSION['ALOID'], $d, $e));
        $res = '<div class="alert alert-success alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button><h4><i class="icon fa fa-check"></i> Successfully Updated!</h4></div>';
    }
    return $res;
}
function addgeneral($landingtitle,$image,$landingcontent,$heading1, $content1, $content2, $content3, $map, $Footer_Block, $facebook, $beforehead, $afterbody, $copyrights, $og_tag, $ip, $id) {
    ...
    $resa->execute(array(trim($landingtitle),trim($image),trim($landingcontent),trim($heading1), trim($content1), trim($content2), trim($content3), trim($map),trim($Footer_Block), trim($og_tag), trim($facebook), trim($copyrights), trim($beforehead), trim($afterbody), trim($ip), $_SESSION['ALOID'], $_SESSION['type'], $id));

    $htry = $db->prepare("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES (?,?,?,?,?,?)");
    $htry->execute(array('General Settings', 2, 'Update', $_SESSION['ALOID'], $ip, $id));
    $res = '<div class="alert alert-success alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button><h4><i class="icon fa fa-check"></i> Successfully Updated!</h4></div>';
    return $res;
}
...
function addhomebanners($title, $externallink, $image_name, $image_alt, $image, $status, $ip, $id) {
    ...
    try {
        ...
        $resa->execute(array(trim($title), trim($externallink), trim($image), trim($image_name), trim($image_alt), trim($status),  trim($ip), $_SESSION['ALOID'], $_SESSION['type'], $id));

        $htry = $db->prepare("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES (?,?,?,?,?,?)");
        $htry->execute(array('Home Banners', 3, 'Update', $_SESSION['ALOID'], $ip, $id));
        $res = '<div class="alert alert-success alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button><h4><i class="icon fa fa-check"></i> Successfully Updated!</h4></div>';
    } catch (Exception $ex) {
...
function addhomecontent($title1, $content1, $status1, $title2, $content2, $status2, $title3, $content3, $status3, $ip, $id) {
    ...
    $resa->execute(array(trim($title1), trim($content1), trim($title2), trim($content2), trim($status1), trim($status2), trim($title3), trim($content3), trim($status3), trim($ip), $_SESSION['ALOID'], $_SESSION['type'], $id));

    $htry = $db->prepare("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES (?,?,?,?,?,?)");
    $htry->execute(array('Home Content', 5, 'Update', $_SESSION['ALOID'], $ip, $id));
    $res = '<div class="alert alert-success alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button><h4><i class="icon fa fa-check"></i> Successfully Updated!</h4></div>';
    return $res;
}
...
function addhomeblocks($title1, $image_name1, $image_alt1, $image1, $button_name1, $button_link1, $status1, $title2, $image_name2, $image_alt2, $image2, $button_name2, $button_link2, $status2, $ip, $id) {
    ...
    try {
        ...
        $resa->execute(array(trim($title1), trim($image1), trim($image_name1), trim($image_alt1), trim($button_name1), trim($button_link1), trim($status1), trim($title2), trim($image2), trim($image_name2), trim($image_alt2), trim($button_name2), trim($button_link2), trim($status2), trim($ip), $_SESSION['ALOID'], $_SESSION['type'], $id));

        $htry = $db->prepare("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES (?,?,?,?,?,?)");
        $htry->execute(array('Home Blocks', 3, 'Update', $_SESSION['ALOID'], $ip, $id));
        $res = '<div class="alert alert-success alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button><h4><i class="icon fa fa-check"></i> Successfully Updated!</h4></div>';
    } catch (Exception $ex) {
        $res = '<div class="alert alert-success alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button><h4><i class="icon fa fa-check"></i>' . $ex . '</h4></div>';
    }
...
function addsocialmedia($social_media, $link, $order, $status, $ip, $id) {
    ...
    $resa->execute(array(trim($social_media), trim($link), trim($order), trim($status), trim($ip), $_SESSION['ALOID'], $id));

    $htry = $db->prepare("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES (?,?,?,?,?,?)");
    $htry->execute(array('Social Media', 7, 'Update', $_SESSION['ALOID'], $ip, $id));
    $res = '<div class="alert alert-success alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button><h4><i class="icon fa fa-check"></i> Successfully Updated!</h4></div>';
    return $res;
}
...
function addbreadcrumb_banner($title1, $link1, $content1, $imagealt1, $imagename1, $image1, $title2, $link2, $content2, $imagealt2, $imagename2, $image2, $ip, $thispageid, $getid) {
    ...
    //$ress="UPDATE `homebanners` SET `title1`='".$title1."', `link1`='".$link1."', `content1`='".$content1."', `image_alt1`='".$imagealt1."', `image_name1`='".$imagename1."',`image1`='".$image1."',`title2`='".$title2."',`link2`='".$link2."',`content2`='".$content2."',`image_alt2`='".$imagealt2."', `image_name2`='".$imagename2."',`image2`='".$image2."',`title3`='".$title3."',`link3`='".$link3."',`content3`='".$content3."', `image_alt3`='".$imagealt3."', `image_name3`='".$imagename3."',`image3`='".$image3."', `ip`='".$ip."', `updated_by`='".$_SESSION['ALOID']."' WHERE `hbid`='".$getid."'";
    ...
    $resa->execute(array(trim($title1), trim($link1), trim($content1), trim($imagealt1), trim($imagename1), trim($image1), trim($title2), trim($link2), trim($content2), trim($imagealt2), trim($imagename2), trim($image2), trim($ip), $_SESSION['ALOID'], $getid));
    $htry = $db->prepare("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES (?,?,?,?,?,?)");
    $htry->execute(array('Home Banners', $thispageid, 'Update', $_SESSION['ALOID'], $ip, $id));
...
function addcaptcha($sitekey, $secret, $status, $ip, $cid) {
    ...
    if ($cid == '') {
        ...
        $resa->execute(array($sitekey, $secret, $ip, $status, $_SESSION['ALOID'], $_SESSION['UIDD']));
        $insert_id = $db->lastInsertId();

        $htry = $db->prepare("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES (?,?,?,?,?,?)");
        $htry->execute(array('Captcha Mgmt', '35', 'Insert', $_SESSION['ALOID'], $_SERVER['REMOTE_ADDR'], trim($insert_id)));
        $res = '<div class="alert alert-success alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button><h4><i class="icon fa fa-check"></i>Successfully Inserted</h4></div>';
    } else {
        $resa = $db->prepare("UPDATE `captcha` SET `sitekey`=?,`secret`=?,`ip`=?,`status`=?,`updated_by`=?,`updated_type`=? WHERE `cid`=?");
        $resa->execute(array($sitekey, $secret, $ip, $status, $_SESSION['ALOID'], $_SESSION['UIDD'], $cid));
        $htry = $db->prepare("INSERT INTO `history` (`page`,`pageid`,`action`,`userid`,`ip`,`actionid`) VALUES (?,?,?,?,?,?)");
        $htry->execute(array('Captcha Mgmt', '35', 'Update', $_SESSION['ALOID'], $_SERVER['REMOTE_ADDR'], trim($cid)));
        $res = '<div class="alert alert-success alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button><h4><i class="icon fa fa-check"></i>Successfully Updated</h4></div>';
    }
    return $res;
}
